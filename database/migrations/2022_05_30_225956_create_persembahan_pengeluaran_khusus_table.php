...
class CreatePersembahanPengeluaranKhususTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('persembahan_pengeluaran_khusus', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('kategori_id')->unsigned();
            $table->foreign('kategori_id')->references('id')->on('detail_kategori')->onDelete('cascade')->onUpdate('cascade');

            $table->string('nama_pengguna'); 
            $table->string('kode_persembahan_pengeluaran_khusus');      
            $table->date('tanggal');
    ...
    {
        Schema::dropIfExists('persembahan_pengeluaran_khusus');
    }
}
